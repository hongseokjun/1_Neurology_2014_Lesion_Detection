...
excluded_patient_cases = { '016', '020', '05315T' }';
...
data_dir = '/local_raid/seokjun/01_project/IntracorticalAnalysis/03_result/99_1.5T/control/';
...
%% Feature: RI_IntraCortical (vertexnum x 5 x numPatient), 
%%          PG_IntraCortical (vertexnum x 5 x numPatient),
%%          PG_GM_WM         (vertexnum x numPatient),
%%          CT, MC, SD       (vertexnum x numPatient, only T1)
...
T1_RI_wCortical_temp2        = zeros(size(case_num_pat, 1), vertexnum);

data_dir = '/local_raid/seokjun/01_project/IntracorticalAnalysis/03_result/99_1.5T/FCD/';
file_postfix = [ '_quadratic_sm_5_rsl.txt' ];
for j = 1 : size(case_num_pat, 1)
    ...
    T1_RI_wCortical_temp2(j, 1:vertexnum) = mean(T1_RI_IntCortical_temp2([2 3 4 5], 1:vertexnum, j), 1);
    fprintf([ 'T1: RI_corrected, PG, TG in Inc, ' ]);    
...
T1_RI_wCortical_z           = zeros(size(case_num_pat, 1), vertexnum);
...
for j = 1 : size(case_num_pat, 1)
    ...
    T1_RI_wCortical_z(j, 1:vertexnum)     = (T1_RI_wCortical_temp2(j, 1:vertexnum) - T1_RI_wCortical_mean) ./ T1_RI_wCortical_std;
...
IntMask = repmat(TemplateMask, [5, 1]);
SubMask = repmat(TemplateMask, [3, 1]);
OUTPATH_temp = '/local_raid/seokjun/01_project/IntracorticalAnalysis/03_result/zscoremap_frontiers/t1_1.5T/FCD/';
for j = 1 : size(case_num_pat, 1)
    ...
    exportfigbo(f,[OUTPATH_temp '/' Prefix_cont '_' case_num_pat{j} '_z-score_T1_Intra_RIw.png' ], 'png', 6); close(f);    
    ...
    exportfigbo(f,[OUTPATH_temp '/' Prefix_cont '_' case_num_pat{j} '_z-score_T1_Intra_RI.png' ], 'png', 6); close(f);
    ...
    exportfigbo(f,[OUTPATH_temp '/' Prefix_cont '_' case_num_pat{j} '_z-score_T1_Intra_PG.png' ], 'png', 6); close(f);
    ...
    exportfigbo(f,[OUTPATH_temp '/' Prefix_cont '_' case_num_pat{j} '_z-score_T1_GM-WM_PG.png' ], 'png', 6); close(f);
    ...
    exportfigbo(f,[OUTPATH_temp '/' Prefix_cont '_' case_num_pat{j} '_z-score_T1_Mid_CT.png' ], 'png', 6); close(f);
    ...
    exportfigbo(f,[OUTPATH_temp '/' Prefix_cont '_' case_num_pat{j} '_z-score_T1_Mid_MC.png' ], 'png', 6); close(f);
    ...
    exportfigbo(f,[OUTPATH_temp '/' Prefix_cont '_' case_num_pat{j} '_z-score_T1_WM_SD.png' ], 'png', 6); close(f);

end

OUTPATH = '/local_raid/seokjun/01_project/IntracorticalAnalysis/03_result/zscoremap_frontiers/t1_1.5T/FCD/';
post_fix = [ '_' Parametric '_sm_' num2str(Kernel) '_pat.png' ];
z_score_database = zeros(size(case_num_pat, 2), vertexnum, 5);
f = figure; 
...
%     SurfStatView(T1_RI_wCortical_z(i, :)-T1_PG_gw_IntCortical_z(i, :)+T1_CT_midCortical_z(i, :)+T1_SD_wmCortical_z(i, :)+T1_MC_midCortical_z(i, :), TemplateSurf{3}, case_num_pat{i});
%     SurfStatColLim([-10 10]);
%     exportfigbo(f,[OUTPATH '/mcd_' case_num_pat{i} '_z-score_total.png'], 'png', 6); 
end
close(f);

lesion_db_fcd15T = zeros(size(case_num_pat', 1), vertexnum);
for i = 1 : size(case_num_pat, 1)
    surf_dir = '/data/noel/noel6/seokjun/CTFCD-1.2.0_64_1.5T/Lesion/lesion_surf/';
    left_label  = SurfStatReadData1([surf_dir '/mcd_' case_num_pat{i} '_label_union_left_rsl.txt' ]);
    right_label = SurfStatReadData1([surf_dir '/mcd_' case_num_pat{i} '_label_union_right_rsl.txt' ]);   
    lesion_db_fcd15T(i, :) = [left_label right_label];
end