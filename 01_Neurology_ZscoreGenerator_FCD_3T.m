...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
OutDir                    = '/local_raid/seokjun/01_project/IntracorticalAnalysis/old/'
OutDir2                   = '/data/noel/noel6/seokjun/01_project/IntracorticalAnalysis/03_result/';
Group_cont                = 'control'
Prefix_cont               = 'TLE'
Cases_cont                = '/local_raid/seokjun/01_project/IntracorticalAnalysis/01_analysis/Demographic_data_control.txt'
Group_pat                 = 'FCD'
Prefix_pat                = 'mcd'
Cases_pat                 = '/local_raid/01_project/IntracorticalAnalysis/01_analysis/Demographic_data_FCD.txt'
...
cont_case = { '301_1', '303_1', '304_1', '305_1', '306_1', '307_1', '308_1', '309_1', '310_1', '311_1', '314_1', '316_1', '317_1', '321_1', '323_1', '329_1', '332_1', '324_1', '333_1', '334_1', '335_1', '337_1', '338_1', '339_1', '340_1'  };
...
for i = 1 : size(pat_idx', 1)
    for l_r = 1 : size(Left_Right, 2)
        ...
        for s = 1 : NumIntSurf + 2
            switch s
                case 1                        
                    ...
                    TG_IntraCortical_temp{s}(i, Left_Right{2, l_r}) = SurfStatReadData({ [ postfix_temp '_tg_' Postfix2 ] });
        ...
        for s = 1 : NumSubSurf
            ...
            TG_SubCortical_temp{s}(i, Left_Right{2, l_r}) = SurfStatReadData({ [ postfix_temp '_tg_' Postfix2 ] });
        end      
        
        postfix_temp = [ OutDir '/' case_num_pat{pat_idx(i)} '/measurement/' Prefix_pat '_' case_num_pat{pat_idx(i)} '_white_surface_' Postfix1 ]
        PG2_temp(i, Left_Right{2, l_r}) = SurfStatReadData({ [ postfix_temp '_pg2_' Postfix2 ] });
        ...
        if(strcmp(img_contrast{1}, 't1'))
            ...
            postfix_temp = [ OutDir '/' case_num_pat{pat_idx(i)} '/measurement/' Prefix_pat '_' case_num_pat{pat_idx(i)} '_white_surface_' Postfix1 ]             
            SD_temp(i, Left_Right{2, l_r}) = SurfStatReadData({ [ postfix_temp '_sd_' Postfix2 ] });
            postfix_temp = [ OutDir2 '/' case_num_pat{pat_idx(i)} '/' Prefix_pat '_' case_num_pat{pat_idx(i)} '_sulcalGeoDepth_' Left_Right{1, l_r} '_sm_' num2str(Kernel) '_rsl.txt' ];
            SD2_temp(i, Left_Right{2, l_r}) = SurfStatReadData({ [ postfix_temp ] });
        end
    end
end  
...
for i = 1 : size(pat_idx', 1)
    ...
    for l_r = 1 : size(Left_Right, 2)
        ...
        for s = 1 : NumIntSurf + 2
            ...
            TG_IntraCortical_z{s}(i, Left_Right{2, l_r}) = (TG_IntraCortical_temp{s}(i, Left_Right{2, l_r}) - TG_IntraCortical_mean{s}(Left_Right{2, l_r}))./TG_IntraCortical_std{s}(Left_Right{2, l_r});
        ...
        for s = 1 : NumSubSurf
            ...
            TG_SubCortical_z{s}(i, Left_Right{2, l_r}) = (TG_SubCortical_temp{s}(i, Left_Right{2, l_r}) - TG_SubCortical_mean{s}(Left_Right{2, l_r}))./TG_SubCortical_std{s}(Left_Right{2, l_r});
        ...
        if(strcmp(img_contrast{1}, 't1'))
            ...
            SD_z(i, Left_Right{2, l_r}) = (SD_temp(i, Left_Right{2, l_r}) - SD_mean(Left_Right{2, l_r}))./SD_std(Left_Right{2, l_r});
...
mask = SurfStatMaskCut(TemplateSurf);
for i = 1 : size(pat_idx', 1)
    RI_intra_z = [];
    PG_intra_z = [];
    TG_intra_z = [];
    ...
    f = figure; SurfStatView(RI_wCortical_z(i, :).*mask, SurfIntra{3}, case_num_pat{pat_idx(i)});  axis off; material dull;
    SurfStatColLim([-2 2]);
    exportfigbo(f,[OUTPATH '/' img_contrast{1} '/RI/' case_num_pat{pat_idx(i)} '_zscore_RI.png'], 'png', 6); close(f);
    
    f = figure; SurfStatView(-PG2_z(i, :).*mask, SurfIntra{3}, case_num_pat{pat_idx(i)});  axis off; material dull;
    SurfStatColLim([-5 5]);
    exportfigbo(f,[OUTPATH '/' img_contrast{1} '/PG/' case_num_pat{pat_idx(i)} '_zscore_PG.png'], 'png', 6); close(f);    

    f = figure; SurfStatView(CT_z_temp.*mask, SurfIntra{3}, case_num_pat{pat_idx(i)});  axis off; material dull;
    SurfStatColLim([-5 5]);
    exportfigbo(f,[OUTPATH '/' img_contrast{1} '/CT/' case_num_pat{pat_idx(i)} '_zscore_CT.png'], 'png', 6); close(f);        
    
    f = figure; SurfStatView(SD2_z(i, :).*mask, SurfIntra{3}, case_num_pat{pat_idx(i)});  axis off; material dull;
    SurfStatColLim([-5 5]);
    exportfigbo(f,[OUTPATH '/' img_contrast{1} '/SD/' case_num_pat{pat_idx(i)} '_zscore_SD.png'], 'png', 6); close(f);        
    
    f = figure; SurfStatView(MC_z_temp.*mask, SurfIntra{3}, case_num_pat{pat_idx(i)});  axis off; material dull;
    SurfStatColLim([-5 5]);
    exportfigbo(f,[OUTPATH '/' img_contrast{1} '/MC/' case_num_pat{pat_idx(i)} '_zscore_MC.png'], 'png', 6); close(f);            
...
%     
%     f = figure; CSFSurfStatViewDataINC(RI_intra_z, SurfIntra);
%     CSFSurfStatViewColLim([-5 5]);    
%     exportfigbo(f,[OUTPATH '/' img_contrast{1} '/' case_num_pat{pat_idx(i)} '_RI_zscore_features.png'], 'png', 6); close(f);
end


lesion_db = zeros(size(pat_idx', 1), vertexnum);
for i = 1 : size(pat_idx', 1)
    surf_dir = '/data/noel/noel6/CTFCD-1.2.0_64/lesion_surf/';
    left_label  = SurfStatReadData1([surf_dir '/mcd_' case_num_pat{pat_idx(i)} '_label_union_left_rsl.txt' ]);
    right_label = SurfStatReadData1([surf_dir '/mcd_' case_num_pat{pat_idx(i)} '_label_union_right_rsl.txt' ]);
    f = figure; SurfStatView([left_label right_label], SurfIntra{3}, [case_num_pat{pat_idx(i)} ' lesion label'] );
    exportfigbo(f,['/data/noel/noel6/CTFCD-1.2.0_64/lesion_surf/figure/mcd_' case_num_pat{pat_idx(i)} '_lesion_label.png'], 'png', 6); close(f);
    lesion_db(i, :) = [left_label right_label];
end