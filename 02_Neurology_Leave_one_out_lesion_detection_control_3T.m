...
%  3) This script is for estimating the specificity of the classification in controls 
...
load('/local_raid/seokjun/01_project/IntracorticalAnalysis/01_analysis/lesion_detection/Result_control_3T.mat')
...
cont_outlier = [ 5 ];              %% Control cases that will be excluded in this analysis
...
OUTPATH = '/data/noel/noel6/seokjun/01_project/IntracorticalAnalysis/03_result/lesion_detection_frontiers/08_result/';
...
%     exportfigbo(f,[OUTPATH '/' cont_case{j} '_first_classification.png' ], 'png', 6); close(f);
    ...
    if(~isempty(clusid))
        for t = 1 : size(clusid, 2)
            cluster_temp(Clusters_cont(:, j) == clusid(t)) = 1;
            cluster_info{j, 2} =  [ cluster_info{j, 2} sum(Clusters_cont(:, j) == (bLesion_Pt_cont(cluster_set{m, 1, j, p}, j) - 500)) ];
        end
